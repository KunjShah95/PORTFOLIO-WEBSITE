  ...
  return (
    <div className="pt-48 pb-32 min-h-screen">
      <div className="container-aligned space-y-24">
        {/* Aligned Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-border pb-10 gap-10">
            ...
            {activeTab === 'LOGS' ? (
               ...
               >
                  {logs.map((log) => (
                    <div 
                      key={log.hash}
                      className="group relative p-16 border border-border bg-surface/20 flex flex-col lg:grid lg:grid-cols-12 gap-12 items-center hover:border-primary/20 transition-all border-glow"
                    >
                      ...
                      <div className="lg:col-span-6 space-y-4 text-left w-full">
                         ...
                         <h3 className="text-3xl font-black text-txt uppercase tracking-tighter group-hover:text-primary transition-colors leading-none italic">{log.action}</h3>
                         <div className="text-[9px] font-mono text-secondary/60 uppercase tracking-[0.5em] font-black italic">{log.module}</div>
                      </div>
                      <div className="lg:col-span-5 border-l border-border pl-10 w-full">
                         <p className="text-xs text-muted leading-relaxed font-mono italic group-hover:text-txt transition-colors uppercase tracking-tight">
                            {log.details}
                         </p>
                      </div>
                    </div>
               ...
               >
                  {articles.map((art) => (
                    <div 
                      key={art.id}
                      className="group p-16 bg-bg hover:bg-surface/5 flex flex-col md:grid md:grid-cols-12 gap-12 items-center transition-all duration-700"
                    >
                       <div className="md:col-span-1 hidden md:block">
                          <span className="text-3xl font-black text-txt/5 group-hover:text-primary transition-colors italic tracking-tighter">0{art.id}</span>
                       </div>
                       ...
                       <div className="md:col-span-8 space-y-6 w-full">
                          ...
                          <div className="space-y-4">
                             <h3 className="text-4xl md:text-5xl font-black text-txt uppercase tracking-tighter leading-none group-hover:text-primary transition-colors duration-500 italic">
                               {art.title}
                             </h3>
