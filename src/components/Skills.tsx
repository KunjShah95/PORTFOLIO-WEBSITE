        ...
        <div className="grid md:grid-cols-2 gap-px bg-border border border-border overflow-hidden">
          {modules.map((m, i) => (
            <motion.div
              ...
              className="group relative bg-bg p-16 space-y-12 hover:bg-surface/50 transition-colors"
            >
               <div className="flex items-center justify-between">
                  <div className="w-16 h-16 border border-border flex items-center justify-center group-hover:border-primary/40 transition-all bg-surface/20">
                     <m.icon className="w-7 h-7 text-primary opacity-20 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <span className="text-4xl font-black text-txt/5 group-hover:text-primary transition-colors italic tracking-tighter">0{i+1}</span>
               </div>
               
               <div className="space-y-8">
                  <div className="space-y-2 text-left">
                     <h3 className="text-3xl font-black text-txt tracking-widest uppercase italic group-hover:text-primary transition-colors leading-none">{m.title}</h3>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-6 text-left border-t border-border pt-10 mt-auto">
                     {m.caps.map((cap, idx) => (
                        <div key={idx} className="flex items-center gap-3 text-[10px] font-mono text-muted uppercase tracking-[0.2em] group-hover:text-txt transition-colors">
               ...
               <div className="absolute top-0 right-0 w-10 h-10 border-t border-r border-border group-hover:border-primary/20 transition-all translate-x-1 -translate-y-1 group-hover:translate-x-0 group-hover:translate-y-0"></div>
            </motion.div>
          ))}
        </div>
           ...
           <div className="flex flex-wrap items-center justify-center gap-4">
              {['LLM_DISTILLATION', 'MULTIMODAL_RAG', 'QUANTUM_LOGIC', 'MLOPS_GOVERNANCE', 'GRAPH_NEURAL_NETS'].map((topic, i) => (
                 <motion.div 
                    ...
                    className="px-6 py-3 border border-border bg-surface/5 text-[10px] font-black font-mono text-muted hover:text-primary hover:border-primary/40 transition-all cursor-crosshair uppercase tracking-widest italic"
                 >
                    {topic}
                 </motion.div>
              ))}
